...
- **`cvhonors` (used by Program Committees, Honors) was built on `tabular*`, an atomic unbreakable table.** A long entry list either fit entirely on a page or moved as one block to the next, stranding the section header alone above a mostly blank page. `cventries` (Experience/Publications) doesn't have this problem — each `\cventry` is its own small table, so entries can flow across a page break individually. Fixed by converting `cvhonors` to `longtable`, which supports breaking across pages. If you introduce another `tabular*`-based repeating-entry environment, expect the same failure mode under content growth.
